...
sns.lineplot(data=pred_daily, x="Date", y="Predicted_Price_Adjusted", label="Predicted Price (Adjusted)")
plt.title("검증 기간 커피 가격 예측 결과 (보정된 시작 가격)", fontsize=16)
plt.xlabel("Date")
...
# 13. test 성능 (shift 사용 X, 원래 보정 기준)
...
sns.lineplot(data=test_daily[:-1], x="Date", y="Predicted_Price_Adjusted_Shifted", label="Predicted Price (Shifted)")
plt.title("테스트 기간 커피 가격 예측 결과 (보정 + 하루 Shift)", fontsize=16)
plt.xlabel("Date")
plt.ylabel("Coffee Price (USD)")
plt.legend()
plt.grid(True)
