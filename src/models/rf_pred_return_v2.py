...
class Testmodel:
    ...
    def run_training_pipeline(self, save_path="../model_result/rf/rf_pred_result_v2.csv", alpha=5.0):
        df = self.data.copy()
    ...
    def predict_future_until(self, start_date: str, end_date: str, save_path="../model_result/rf/future_pred_v2.csv", alpha=5.0):
        df = self.data.copy()
        df["Date"] = pd.to_datetime(df["Date"])
        df = df.sort_values("Date").reset_index(drop=True).dropna()
